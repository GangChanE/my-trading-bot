...
# =====================================================================
# 🚀 2. 스트림릿 전용 캐싱 데이터 로더 (웹 속도 최적화)
# =====================================================================
@st.cache_data(ttl=86400) # 하루 1번만 다운로드하여 캐싱
def load_and_preprocess_data(start_date="2018-01-01", end_date="today"):
    tickers = list(PORTFOLIO_CONFIG.keys())
    df_all = yf.download(tickers, start=start_date, end=end_date, progress=False)['Close']
...
def run_hybrid_portfolio(df_all, sigma_data, slope_data, start_capital=20000000):
    ...
    for i in range(LR_WINDOW, len(dates)):
        ...
        # [2] 매수 대상 색출 (신규 시그널이 뜰 때만 리밸런싱 가동)
        new_signals = []
        held_tickers = []
        for ticker, p_info in port.items():
            price = df_all[ticker].values[i]
